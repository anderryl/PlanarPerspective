...
class CompressionHandlerArchaic {
    unowned var level: LevelView
    let polygons: [Polygon]
    var cached: [Polygon : [Line]] = [:]
    ...
    func compress(with transform: Transform, reverse: Bool) -> [Line] {
        ...
        let test: Polygon = Polygon(vertices: [Vertex(x: 0, y: 0, z: 0), Vertex(x: 100, y: 100, z: 100), Vertex(x: 134, y: 153, z: 948)])
        if let ret = cached[transform(test)] {
            return ret
        }
    ...
    */
    ...
    /*
    func compress(with transform: Transform, reverse: Bool) -> [Line] {
        var clips = level.polygons.map { transform($0) }
        var lines = clips.map { $0.edges() }
        var next: [[Edge]] = []
        
        outter: for clip in clips {
            inner: for edge in lines {
                var current: [Edge] = []
                //lines[index.y] = MetalEdge{{MetalSegment{MetalVertex{69, 69, 69}, MetalVertex{69, 69, 69}}}, 1};
                //return;
                if (clip.vertices.count == 2) {
                    continue outter;
                }
                var attempted: Bool = false
                for sub in 0 ..< edge.count {
                    var intersections: [CGFloat] = [0]
                    var intercount = 1
                    var segment: Edge = edge[sub]
                    var list: [Edge] = []
                    for i in 0 ..< clip.vertices.count - 1 {
                        list.append(Edge(origin: clip.vertices[i], outpost: clip.vertices[i + 1]))
                    }
                    list.append(Edge(origin: clip.vertices[clip.vertices.count - 1], outpost: clip.vertices[0]))
                    var winding = 0;
                    var dx = segment.outpost.x - segment.origin.x
                    var dy = segment.outpost.y - segment.origin.y
                    var dz = segment.outpost.z - segment.origin.z
                    for i in 0 ..< clip.vertices.count {
                        var clipline: Edge = list[i];
                        var delta2x = clipline.outpost.x - clipline.origin.x
                        var delta2y = clipline.outpost.y - clipline.origin.y
             
                        // create a 2D matrix from the vectors and calculate the determinant
                        var determinant = dx * delta2y - delta2x * dy
             
                        if abs(determinant) < 0.0001 {
                            continue;
                        }
                        //If the coefficients are between 0 and 1, there is an intersection
                        var ab = ((segment.origin.y - clipline.origin.y) * delta2x - (segment.origin.x - clipline.origin.x) * delta2y) / determinant
                        if (ab > 0 && ab < 1) {
                            var cd = ((segment.origin.y - clipline.origin.y) * dx - (segment.origin.x - clipline.origin.x) * dy) / determinant
                            if (cd > 0 && cd < 1) {
                                intersections.append(ab)
                                intercount += 1
                                if (!attempted) {
                                    var sz = ab * dz + segment.origin.z;
                                    var cz = cd * (clipline.outpost.z - clipline.origin.z) + clipline.origin.z;
                                    if (sz < cz) {
                                        continue inner;
                                    }
                                    attempted = true;
                    ...
                    intercount += 1
                    var outside = (winding % 2 == 0)

                    if (intercount == 2 && !outside && clip.vertices.count > 2) {
                        var a1 = clip.vertices[1].x - clip.vertices[0].x
                        var b1 = clip.vertices[1].y - clip.vertices[0].y
                        var c1 = clip.vertices[1].y - clip.vertices[0].y
                        var a2 = clip.vertices[2].x - clip.vertices[0].x
                        var b2 = clip.vertices[2].y - clip.vertices[0].y
                        var c2 = clip.vertices[2].z - clip.vertices[0].z
                        var a = b1 * c2 - b2 * c1;
                        var b = a2 * c1 - a1 * c2;
                        var c = a1 * b2 - b1 * a2;
                        var d = (-a * clip.vertices[0].x - b * clip.vertices[0].y - c * clip.vertices[0].z)
                        //ax + by + cz + d = 0
                        //z = (ax + by + d) / -c
                        var z = (a * segment.origin.x + b * segment.origin.y + d) / -c
                    ...
                    for step in 0 ..< intercount - 1 {
                      var swapped = false;
                        for i in 0 ..< intercount - step - 1 {
                    ...
                    for i in 0 ..< intercount - 1 {
                        if (outside) {
                            var origin = Vertex(x: intersections[i] * dx + segment.origin.x, y: intersections[i] * dy + segment.origin.y, z: intersections[i] * dz + segment.origin.z)
                            var outpost = Vertex(x: intersections[i + 1] * dx + segment.origin.x, y: intersections[i + 1] * dy + segment.origin.y, z: intersections[i + 1] * dz + segment.origin.z)
                            //bools[index.y] = false;
                            current.append(Edge(origin: origin, outpost: outpost))
                        }
                        outside = !outside;
                    }
                }
        ...
        var ret: [Line] = []
        lines.forEach { (array) in
            array.forEach { (edge) in
                ret.append(edge.flatten())
            }
        }
        return ret
    }*/
