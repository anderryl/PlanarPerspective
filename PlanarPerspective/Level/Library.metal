    ...
    for (uint c = 0; c < bounds[0].x; c++) {
        ...
        for (int sub = 0; sub < edge.count; sub++) {
            ...
            for (int i = 0; i < clip.count; i++) {
                ...
                //If it is zero or very close, the lines are parallel, so move on
                ...
                if (ab > 0 && ab < 1) {
                    ...
                    if (cd > 0 && cd < 1) {
                        ...
                        //If there is an intersection on the first segment, increment the winding number
                        if (sub == 0) {
                            winding++;
                        }
                    }
                }
                //If the intersection occurs forward of the origin on the first segment, check the clipping
                else if (ab > 0 && sub == 0) {
                    float cd = ((segment.origin.y - clipline.origin.y) * dx - (segment.origin.x - clipline.origin.x) * dy) / determinant;
